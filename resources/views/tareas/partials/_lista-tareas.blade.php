<ul class="space-y-3 p-1">
    @forelse ($tareas as $tarea)
        <li class="group flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-white rounded-2xl border border-slate-100 transition-all hover:shadow-xl hover:shadow-slate-200/50 hover:border-blue-100
                ...
                <div class="flex flex-col min-w-0">
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        <div class="flex items-center gap-1.5 shrink-0">
                            {{-- Badge Categoría --}}
                            <span
                                class="text-[9px] font-black uppercase tracking-widest text-slate-400 bg-slate-100 px-2 py-0.5 rounded-md">
                                {{ $tarea->category?->name ?? 'General' }}
                            </span>
            ...
            {{-- Acciones --}}
            <div
                class="flex items-center gap-1 mt-3 sm:mt-0 ml-auto sm:ml-0 opacity-100 sm:opacity-100 group-hover:opacity-100 transition-all duration-200 sm:translate-x-2 group-hover:translate-x-0">
                ...
                <form action="{{ route('tareas.destroy', $tarea->id) }}" method="POST">
                    ...
                    <button type="submit"
                        ...
                        onclick="return confirm('¿Borrar la tarea : {{ $tarea->nombre }}?')">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
